...
export class BsOffcanvasComponent implements OnDestroy {
  constructor(@Inject(OFFCANVAS_CONTENT) contentTemplate: TemplateRef<any>) {
    ...
    this.width$ = combineLatest([this.position$, this.size$]).pipe(
      map(([position, size]) => {
    ...
    this.height$ = combineLatest([this.position$, this.size$]).pipe(
      map(([position, size]) => {
    ...
    this.showBackdrop$ = combineLatest([this.hasBackdrop$, this.show$]).pipe(
      map(([hasBackdrop, show]) => hasBackdrop && show)
    );
  }
